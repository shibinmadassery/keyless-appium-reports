...
module.exports = {
  ...
  globals: {
    reporter: function(results, done) {
      ...
      Object.entries(results.modules || {}).forEach(([modPath, mod]) => {
        const name = path.basename(modPath, '.js');
        const testcases = {};
        let mPass = 0, mFail = 0, mSkip = 0;

        Object.entries(mod.completed || {}).forEach(([tcName, tc]) => {
          const isFail = tc.status === 'fail' || (tc.errors || 0) > 0;
          const isSkip = tc.status === 'skip';
          const failedAssert = (tc.assertions || []).find(a => !a.passed);

          testcases[tcName] = {
            passed:     tc.passed  || 0,
            failed:     tc.failed  || 0,
            errors:     tc.errors  || 0,
            skipped:    tc.skipped || 0,
            timeMs:     tc.timeMs  || 0,
            assertions: (tc.assertions || []).map(a => ({
              name:    a.name    || '',
              message: a.message || '',
              passed:  !!a.passed,
              failure: a.failure || (!a.passed ? a.message : null) || null
            }))
          };
          if (failedAssert) {
            testcases[tcName].failure = failedAssert.failure || failedAssert.message;
          } else if (isFail) {
            const err = tc.lastError;
            if (err) {
              testcases[tcName].failure = err.message || (typeof err === 'string' ? err : JSON.stringify(err));
              testcases[tcName].stackTrace = tc.stackTrace || err.stack || null;
            }
          }

          if (isSkip)       mSkip++;
          else if (isFail)  mFail++;
          else              mPass++;
        });
        ...
        modules[name] = {
          ...
          failures:       mFail,
          skipped:        mSkip,
          tests:          mPass + mFail + mSkip,
          assertionsCount: mod.assertionsCount || 0,
      ...
      const report = {
        ...
        duration:  Math.round(parseFloat(results.elapsedTime || '0')),
        passed:    totalPassed,
        failed:    totalFailed,
      ...
      fs.writeFileSync(
        path.join(dir, `report-${ts}.json`),
        JSON.stringify(report, null, 2)
      );
      console.log(`\nDashboard report → reports/report-${ts}.json`);
      done();
    }
  },
